...
- Treat `docs/real_annotation_contract.md` as the shared vocabulary for real annotations and morphology reviews. Schema 0.7 separates loss-eligible supervision from latent geometry provenance; never use latent bundle-child or clump-fragment arrays as supervised defaults.
- For the high-level STED morphology measurement objective and first-model scope, see `docs/sted_morphology_analysis_plan.md`. Keep real-supervised targets aligned with the expert annotation protocol: fibers, clump, uncertain_ignore, and JFilament snakes; do not require real endpoint/crossing/junction labels.
- Real expert annotation files and real-image-derived overlays are local/private artifacts and must not be committed to the public repository.
- Keep morphology generation condition-blind. Do not select geometry from culture, disease, tau isoform, or DIV metadata, and do not present coarse real-image morphology proxies as fitted biological targets.
